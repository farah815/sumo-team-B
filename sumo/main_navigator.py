...
class PerfectSumoNavigator(Node):
    ...
    def scan_callback(self, msg):
        ...
        self.back_dist = min(back_ranges) if back_ranges else 3.0
        self.right_dist = min(right_ranges) if right_ranges else 3.0
        self.front_dist = min(front_ranges) if front_ranges else 3.0
        self.left_dist = min(left_ranges) if left_ranges else 3.0
    ...
    def control_loop(self):
        ...
        if self.state == 'TURNING':
            ...
            is_junction_opened = (self.left_dist > 1.2) or (self.right_dist > 1.2)
            
            if is_front_blocked or is_junction_opened:
                # تأخير بسيط للقرار لحد ما الروبوت يتوسطن جوه التقاطع وما يلفش بدري
                if is_junction_opened and not is_front_blocked and self.front_dist > 0.8:
                    twist.linear.x = 0.8
                    twist.angular.z = 0.0
                ...
                if valid_choices:
                    ...
                    if 'front' in valid_choices and valid_choices['front'][0] > 0.8:
                        best_dir_name = 'front'
                        best_dist, best_offset, best_deg = valid_choices['front']
                    else:
